...
const getGenericNLS = (request, response, table_name ) => {
...
const getGenericNLSById = (request, response, table_name ) => {
...
const createGenericNLS = (request, response, table_name )=> {
  pool.connect((err, client, done) => {
    ...
    client.query(`BEGIN`, err => {
      ...
      client.query(`INSERT INTO nucl.${table_name}( title ) VALUES ($1) RETURNING id`, [title], (err, res) => {
        if (shouldAbort(err, response)) return;      
        const { id } = res.rows[0];
        console.log(`Id = `+id);
...
const deleteGenericNLS = (request, response, table_name ) => {
  pool.connect((err, client, done) => {
    const shouldAbort = (err, response) => {
      if (err) {
...
const updateGenericNLS = (request, response, table_name ) => {
  pool.connect((err, client, done) => {
    const shouldAbort = (err, response) => {
      if (err) {
    ...
    client.query(`BEGIN`, err => {
      if (shouldAbort(err, response)) return;
      client.query(`UPDATE nucl.${table_name} SET title = $1 WHERE id = $2`, [title, id], (err, res) => {
        if (shouldAbort(err, response)) return;      
        // const { id } = res.rows[0];
        //console.log(`Id = `+id);
