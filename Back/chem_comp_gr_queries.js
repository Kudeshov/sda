...
const getChemCompGr = (request, response ) => {
  pool.query(`SELECT * from ( `+
    `SELECT ccg.id+1000000 as id, title, COALESCE(chelement_id,1) as parent_id, formula, ccgn1.name name_rus, ccgn2.name name_eng, ccgn1.descr descr_rus, ccgn2.descr descr_eng FROM nucl.chem_comp_gr ccg `+
    ...
    `UNION SELECT 1 as id, 'Не определено' as Title, null as parent_id, null as formula, null as name_rus, null as name_eng, null as descr_rus, null as descr_eng `+
    `) as chelement_chem `+
